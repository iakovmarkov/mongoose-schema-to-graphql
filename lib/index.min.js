...
`)}return{type:refGraphQLType}});return}var pathInstanceName=path.instance;if(pathInstanceName==='Embedded'){if(parsedArgs.schema===path.schema){setResultingTypeFieldFn(pathName,function(){return{type:resultingGraphQLType}})}else{setResultingTypeField(pathName,{type:createType(_extends({name:generateNameForSubField(resultingGraphQLOptions.name,pathName),description:generateDescriptionForSubField(resultingGraphQLOptions.name,pathName),class:parsedArgs.class,schema:path.schema,exclude:parsedArgs.exclude},nestedSchemaArgs&&nestedSchemaArgs[pathName]||{}),nestedSchemaArgs)})}return}if(pathInstanceName==='Array'){if(path.schema){if(parsedArgs.schema===path.schema){setResultingTypeFieldFn(pathName,function(){return{type:new _graphql.GraphQLList(resultingGraphQLType)}})}else{setResultingTypeField(pathName,{type:new _graphql.GraphQLList(createType(_extends({name:generateNameForSubField(resultingGraphQLOptions.name,pathName),description:generateDescriptionForSubField(resultingGraphQLOptions.name,pathName),class:parsedArgs.class,schema:path.schema,exclude:parsedArgs.exclude},nestedSchemaArgs&&nestedSchemaArgs[pathName]||{}),nestedSchemaArgs))})}}else{var arrayElementInstanceName=path.caster.instance;var resType=new _graphql.GraphQLList(convertPrimitiveObjectInstanceToGraphQLType(arrayElementInstanceName));setResultingTypeField(pathName,{type:resType})}return}setResultingTypeField(pathName,{type:convertPrimitiveObjectInstanceToGraphQLType(pathInstanceName)})});extendResultingTypeField(parsedArgs.extend);extendResultingTypeField(parsedArgs.props);setFnName(resultingGraphQLOptions.fields,'fields');resultingGraphQLType=generateGraphQLType(parsedArgs.class,resultingGraphQLOptions);memoize(parsedArgs.name,resultingGraphQLType);return resultingGraphQLType}exports.default=createType;
